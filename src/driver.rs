...
use std::{
    collections::{HashMap, HashSet}, fs::File, io::{Cursor, Read, Write}, ops::Deref, path::{Path, PathBuf}, process::Command, rc::Rc, result::Result as StdResult, str::FromStr, time::{Duration, SystemTime}
};
...
struct BridgeState<'a> {
    ...
    // processingsession
    ps: Option<* mut ProcessingSession<'a>>,
}
...
impl IoProvider for BridgeState<'_> {
...
impl DriverHooks for BridgeState<'_> {
    ...
    fn event_output_flushed_imm(
        ...
        let summ = self
            ...
            .get_mut(&_name)
            .expect("closing file that wasn't opened?");
        summ.write_digest = Some(_digest);
        if let Some(lps) = self.ps {
            unsafe { (*lps).write_files(None, _status, true); }
        }
    }
...
pub struct ProcessingSession<'a> {
    // Security settings.
    security: SecuritySettings,
